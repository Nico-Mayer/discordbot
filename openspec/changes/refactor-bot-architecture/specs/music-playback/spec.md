...
When a track finishes for a reason that permits starting the next one, the bot SHALL play the next queued track, and SHALL leave the voice channel when the queue is exhausted.
...
- **THEN** the bot MUST leave the voice channel
