...
Configuration errors SHALL be reported together and MUST prevent the bot from starting. The process MUST exit with a non-zero status.
...
On receiving an interrupt or termination signal the bot SHALL shut down in an orderly way: stop accepting new work, leave any voice channels it is connected to, close the Lavalink and gateway connections, and exit with a zero status.
